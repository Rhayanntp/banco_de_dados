 ...
 * informação sobre cunjuntos de linhas especificados
 * AVG
*/
...
-- qual é a medía de preço das pizzas
select avg(valor) as media from pizzas;
select avg(valor) as media from pizzas where nome like '%esa';


-- qual é o valor da pizza mais cara do cardápio sa pizzaria?
select max(valor) as 'maior valor' from pizzas;

-- qual é o valor da pizza mais barata do cardápio sa pizzaria?
...
select pedido_id as pedido, sum(valor) as 'valor pedido', sum(quantidade) as 'qtde pizzas', avg(valor) as media, sum(valor) / sum(quantidade) as 'valor médio'
...
select c.id, c.nome, p.id as pedido from clientes as c inner join pedidos as p on p.cliente_id = c.id;

-- LEFT JOIN (ou LEFT OUTER JOIN): Retorna todos os regidtros da tabela da esquerda
-- (primeira tapela mencionada) e os regidtros correspondentes da tabela da direita
...
-- RIGHT JOIN (ou RIGHT OUTER JOIN): Retorna todos os regidtros da tabela da direita
-- (segunda tapela mencionada) e os regidtros correspondentes da tabela da esquerda
...
-- quantidade de pedidos realizados por cliente, exibir nome do cliente e quantidade de pedidos realizou
